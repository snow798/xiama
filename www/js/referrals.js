...
define(function(require, exports, module){
    ...
    var ref_tpl={
        ...
        newMusic: function(data){   //新碟首发
            ...
            for(var s in data.collections){
                ...
                html += '<li data-href="'+sel.url+'"><div class="cover" style="background-image:url('+sel.logo+EXTEND+');"></div><div class="title">'+sel.name+'</div><div class="author">'+sel.author+'</div></li>';
            }
            html += '</ul>';
        ...
        action: function(data){      //专辑简短推介
            ...
            action.style.backgroundImage= 'url('+data.logo+EXTEND+')';
        ...
        mv: function(data){
            ...
            mv.className= 'sq_mv';
            var sel= data.mv;
            html += '<div class="view">'+sel.play_count+'</div><div class="mask_play"><svg><use xlink:href="#max-player"/></svg></div><div class="itemBody" style="background-image:url('+sel.mv_cover+EXTEND+');"></div><div class="title">'+sel.title+'</div><div class="author">'+sel.artist+'</div>';
            mv.innerHTML= html;
        ...
        rec_song: function(data){
            ...
            html += '<div class="view">'+views+'</div><div class="title">'+sel.reason+'</div><div class="cover" style="background-image: url('+sel.album_logo+EXTEND+');"></div><div class="pinyin">'+sel.song_name+'</div><div class="author">'+sel.singers+'</div><div class="more"><svg class="icon sos-more"><use xlink:href="#sos-more"></use></svg></div>';
            rec.innerHTML= html;
        ...
        featured: function(data){
            ...
            html += '<div class="view">'+views+'</div><div class="cover" style="background-image:url('+sel.collect_logo+EXTEND+');"></div><div class="title">'+sel.collect_name+'</div><div class="title_sub">'+sel.author_name+'</div><div class="tag"></div><div class="content">'+sel.description+'</div>';
        ...
        rec_song2: function(data){
            ...
            html += '<div class="view">'+views+'</div><div class="title">'+sel.reason+'</div><div class="cover" style="background-image: url('+sel.album_logo+EXTEND+');"></div><div class="pinyin">'+sel.album_name+'</div><div class="author">'+sel.artist_name+'</div><div class="more"></div>';
            rec.innerHTML= html;
            ref_cont.appendChild(rec);
        },
        featured2: function(data){
            ...
            html += '<div class="view">'+views+'</div><div class="cover" style="background-image:url('+sel.collect_logo+EXTEND+');"></div><div class="title">'+sel.reason+'</div><div class="title_sub">'+sel.collect_name+'</div><div class="author_name">'+sel.author_name+'</div><div class="tag"></div><div class="content">'+sel.description+'</div>';
            featured.innerHTML= html;
            ref_cont.appendChild(featured);
        }
    };
    ...
    function getBannerData(){
        util.ajax(GLOBAL_URL, function(data){
                ...
                type: 'POST',
                data: {
                    url: 'http://spark.api.xiami.com/api?api_key=263b63d85992a30cc6030aff03c9dfd0&call_id=1451370244889&av=android_101&v=5.0&app_v=5010100&os_v=19_4.4.2&ch=700145&network=1&device_id=353918056359637&platform_id=1&lg=1&utdid=VY0KxJl2HXADAKUwViVKiJ1A&resolution=1280*768&method=music.start&page=1&device_type=Nexus+4&gps=114.401749%2C30.476622&ssid=%22zallds%22&bssid=88%3A25%3A93%3A83%3A61%3A01&proxy=0&api_sig=f83bd70e797dcadb5d07adafe17627fe&access_token=9b80ccb16761a524603f6ec4ad37f5ac'
                }
            }
        );
